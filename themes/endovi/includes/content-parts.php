...
use function endoviTheme\Helpers\get_version;
 ...
 */
function endovi_inline_style( string $name, bool $echo = true ): string {
	global $endovi_used_inline_styles;
	if ( ! is_array( $endovi_used_inline_styles ) ) {
		return '';
	}

	// Check is this stylesheet was already added on the page
	$is_added = in_array( $name, $endovi_used_inline_styles, true );
	if ( ! $is_added ) {
		$endovi_used_inline_styles[] = $name;
	}
	...
	if ( ! $is_added || wp_doing_ajax() ) {
		...
		$path = ENDOVI_THEME_PATH . "build/inline/$name.css";
		...
		if ( $wp_filesystem->exists( $path ) ) {
			...
			// Add unique ID
			$id = 'endovi-' . str_replace( '_', '', $name ) . '-inline-css';
			if ( $is_added ) {
				$id = uniqid( "$id-" );
			}
 ...
 */
function endovi_the_image( $image, string $classes = '', $size = 'full', $extra_attributes = '', bool $print = true ) {
	// Get image classes
 ...
 * @param bool $print Optional. Print or just return the markup.
 */
function endovi_the_sprite_icon( string $icon_id, array $sizes = array(), string $classes = '', bool $print = true ) {
	$ver = get_version();

	$icon_url = ENDOVI_TEMPLATE_URL . "build/sprite.svg?v=$ver#$icon_id";
 ...
 */
function endovi_the_logo() {
	$custom_logo_id        = get_theme_mod( 'endovi_logo' );
	$custom_logo_id_mobile = get_theme_mod( 'endovi_mobile_logo' );
	...
	if ( $custom_logo_id ) {
		$custom_logo_id_mobile = $custom_logo_id_mobile ? $custom_logo_id_mobile : $custom_logo_id;
		?>
		<a class="custom-logo-link desktop img-middle default-hover"
			href="<?php echo esc_url( $home_url ); ?>"
			rel="home">
			<?php endovi_the_image( $custom_logo_id, 'custom-logo' ); ?>
		</a>
		<a class="custom-logo-link mobile img-middle"
			href="<?php echo esc_url( $home_url ); ?>"
			rel="home">
			<?php endovi_the_image( $custom_logo_id_mobile, 'custom-logo' ); ?>
		</a>
		<?php
	}
 ...
 * @param bool $print Optional. Print or just return the markup.
 */
function endovi_the_link( array $link, string $classes = '', string $extra_data = '', bool $print = true ) {
	$link_url    = $link['url'] ?? '#';
