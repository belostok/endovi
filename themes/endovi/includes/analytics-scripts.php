...
namespace endoviTheme\AnalyticsScripts;
...
function add_google_site_verification_tag() {
	$code = get_theme_mod( 'endovi_google_verification' );
	if ( ! $code ) {
		return;
	}
...
function add_fb_pixel_head() {
	$fb_pixel = get_theme_mod( 'endovi_fbpixel' );
	if ( ! $fb_pixel ) {
		return;
	}
...
function add_linkedin_insight_tag() {
	$linkedin_insight_tag_id = get_theme_mod( 'endovi_linkedin_insight' );
